...
<style>
/* Per-analyst left-panel visibility (Calendar Settings -> Left panel, and
   System -> Preferences). 'hover' collapses .calendar-sidebar to a 16px
   hot-zone that expands on hover; 'always' (default) leaves it pinned.
   Mirrors the knowledge / contracts pattern. Pref key: calendar_sidebar_mode. */
.calendar-container { position: relative; }
.calendar-container.sidebar-hover .calendar-sidebar {
    position: absolute;
...
.calendar-container.sidebar-hover .calendar-sidebar::before {
    content: '';
    position: absolute;
    top: 50%;
    left: 6px;
...
.calendar-container.sidebar-hover .calendar-sidebar:hover::before { opacity: 0; }
</style>
<script>
(async function() {
    try {
        const r = await fetch('<?php echo BASE_URL; ?>api/system/get_user_preference.php?key=calendar_sidebar_mode', { credentials: 'same-origin' });
        const d = await r.json();
        const mode = (d.success && d.value === 'hover') ? 'hover' : 'always';
        document.querySelectorAll('.calendar-container').forEach(el => el.classList.toggle('sidebar-hover', mode === 'hover'));
